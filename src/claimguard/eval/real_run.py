...
def _is_under_flagged(transition: str) -> bool:
    """Did the pipeline flag LESS than it should have?

    Derived from the transition rather than listed, so adding or renaming a packaging
    status cannot silently turn this into a false all-clear on the project's one hard
    safety gate. Under- and over-flagging are not symmetric: CLAUDE.md makes "a
    low-confidence code that gets submitted without a human-review flag" a hard failure,
    while over-flagging only costs reviewer time.
    """
    expected, _, actual = transition.partition("->")
    return bool(actual) and actual == "ready" and expected != "ready"
...
def evaluation_markdown(report: dict, *, checkpoint: Path) -> str:
    ...
    n = report["n"]
    # Resolve through settings like every other module, and never invent a denominator:
    # a fabricated "/ 200" on a corpus of unknown size misstates the sample in the one
    # document whose job is to state it honestly.
    total = len(list((Path(get_settings().data_dir) / "golden").glob("*.json")))
    lines = [
        "# Evaluation",
        "",
        ...
        "",
        "Real-provider (`gemini-2.5-flash`) run over the synthetic golden set. "
        + (f"**n = {n} / {total} records.**" if total
           else f"**n = {n} records** (golden corpus not found — denominator unknown)."),
        "",
    ]
    ...
    under = {k: v for k, v in report["packaging_breakdown"].items() if _is_under_flagged(k)}
    lines += [
        "",
