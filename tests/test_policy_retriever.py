...
def test_retriever_scopes_to_insurer_plan():
    ...
    hits = r("room rent deduction", insurer_id="STAR", plan_id="SEC1", k=5)
    assert 1 <= len(hits) <= 5
    # every returned clause_id belongs to the STAR-SEC1 policy
    assert all(c["clause_id"].startswith("STAR-SEC1-") for c in hits)
...
def test_retriever_exact_text_ranks_its_own_clause_first():
    ...
    star = next(p for p in policies if p["insurer_id"] == "STAR")
    target = star["clauses"][3]
    r = PolicyRetriever(policies, embed)
    hits = r(target["text"], insurer_id="STAR", plan_id="SEC1", k=5)
